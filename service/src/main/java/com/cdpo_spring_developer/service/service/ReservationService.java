...
public class ReservationService {
    ...
    public List<StatisticRequest> getDoneReservations() {
        List<Reservation> reservations = reservationRepository.findAllByStatusDone();
        if (reservations.isEmpty()) throw new ServiceException("Записи не найдены");
        return reservations.stream().map(reservationMapper::mapToDTO).toList()
                .stream().map(reservationMapper::mapToRequest).toList();
